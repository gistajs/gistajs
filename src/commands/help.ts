...
export function getHelpText(command?: string) {
  ...
  if (command === 'add') {
    return [
      ...
      '',
      `  ${c.bold('Usage:')}`,
      `    ${c.dim('$')} ${c.bold('gistajs add')} <source>`,
      '',
      `  ${c.bold('Examples:')}`,
      `    ${c.dim('$')} gistajs add ./addons/storage`,
      '',
      `  Planning only for now. It prints file changes, collisions, and touchpoints without writing anything.`,
      '',
    ].join('\n')
  }
